...
| 2 | Database models (6 tables) | 3 | Jul 25 | Manufacturer, Category, Product, OCRResult, Match, CrawlerLog — SQLAlchemy async |
| 3 | Pydantic schemas | 2 | Jul 25 | Request/response schemas for all 11 endpoints |
| 4 | OCR preprocessor | 5 | Jul 25 | OpenCV pipeline: perspective correction, contrast enhancement, rotation correction, resize — multiple iterations |
| 5 | OCR reader (PaddleOCR) | 4 | Jul 25 | PaddleOCR wrapper, upgraded to v3.7 API, fallback handling |
| 6 | OCR parser | 4 | Jul 25 | Regex extraction: manufacturer, model, energy class, heat output, fuel type. Alias dict — validation passes |
| 7 | OCR pipeline orchestrator | 2 | Jul 25 | Full pipeline with fallback (preprocessed → original) |
| 8 | Matching engine | 5 | Jul 25 | RapidFuzz weighted scoring, dynamic redistribution, raw text fallback — threshold tuning |
| 9 | EPREL crawler service (v1) | 4 | Jul 25 | Initial async httpx + BeautifulSoup scraper |
| 10 | REST API endpoints (11) | 5 | Jul 25 | OCR, products, manufacturers, crawler, health, metrics, admin |
| 11 | JWT authentication | 3 | Jul 25 | bcrypt, token generation, auth middleware |
| 12 | Database seeder | 3 | Jul 26 | 72 real-world EPREL products, 28 manufacturers, 9 categories |
| 13 | Tests (34 → 58 tests) | 8 | Jul 26 | OCR parser, matching, crawler, API endpoint tests — async fixture debugging |
| 14 | Docker setup | 3 | Jul 26 | Dockerfile, docker-compose.yml, nginx.conf — port mapping fixes |
| 15 | Bug fixes (v1) | 5 | Jul 26 | numpy.int32 unpack, PaddleOCR v3.7 migration, preprocessing, matching threshold |
| 16 | Standalone frontend | 4 | Jul 26 | HTML/CSS/JS SPA: Upload, Products, Dashboard — separate repo |
| 17 | Documentation (v1) | 4 | Jul 28 | README, API docs, architecture docs, deployment guide, hours log |
| 18 | EPREL API investigation & reverse engineering | 14 | Jul 28 | Public API discovery, endpoint mapping, pagination analysis, rate limits — overnight research session |
| 19 | EPREL crawler rewrite (v2) | 20 | Jul 28 | JSON API integration, field mapping, energy class normalization, fuel type classification, upsert logic, background task — all-nighter |
| 20 | Category-specific edge cases | 14 | Jul 28 | Heat pump type discrimination, fuel fallbacks, category fallbacks, combination heater classification |
| 21 | PostgreSQL setup & migration | 10 | Jul 28 | PostgreSQL 16, asyncpg, connection testing, .env config — late night debugging |
...
| 24 | Dataset annotation planning | 6 | Jul 28 | Annotation schema, Label Studio evaluation, label taxonomy, train/val/test split |
| 25 | OCR pipeline improvements | 3 | Jul 28 | Energy class regex, heat output parsing, fuel type keyword detection |
| 26 | Image preprocessing improvements | 3 | Jul 28 | Grayscale conversion, thresholding, noise reduction |
| 27 | Database matching improvements | 3 | Jul 28 | Match ranking, partial match handling, full-text search |
...
| 31 | Architecture decision records | 16 | Jul 28 | MILESTONE_01_REQUIREMENTS_ARCHITECTURE.md — technology selection, comparison tables, rejection analysis — overnight |
| 32 | Work progress tracking | 3 | Jul 28 | WORK_PROGRESS.md 84h→151h, milestone details, EPREL pagination bug |
| 33 | CORS debugging & configuration | 3 | Jul 29 | Live Server (port 5500) origin, null origin for file://, ALLOWED_ORIGINS in .env |
| 34 | Frontend DB browser — filter bar | 3 | Jul 29 | Category/energy/fuel dropdowns, product count display, search bar enhancement |
| 35 | Frontend DB browser — pagination | 3 | Jul 29 | Page navigation with ellipsis rendering, 50-per-page, total page counter |
| 36 | Frontend DB browser — detail panel | 4 | Jul 29 | Click-to-view product detail, raw JSON expandable, selected row highlight |
| 37 | Backend filter API & categories | 4 | Jul 29 | category/energy_class/fuel_type query params, GET /categories endpoint, list_categories service |
| 38 | EPREL eprel_id lookup endpoint | 2 | Jul 29 | GET /products/eprel/{eprel_id}, raw_json in detail response |
| 39 | Frontend symlink & root serving | 2 | Jul 29 | Symlink HeatScanAI/frontend → HeatScanAI-frontend, root / and /app endpoints |
| 40 | Comprehensive documentation (v3) | 10 | Jul 29 | ARCHITECTURE.md rewrite with full pipeline diagrams, API.md update with all current endpoints — late night |
| 41 | Seed product cleanup | 3 | Jul 29 | Deleted 72 fake EPREL-* seeds + 28 orphaned matches/mfrs, 534 real products remain |
| 42 | Matching threshold tuning | 2 | Jul 29 | Threshold 5.0→30.0, primary match gate (mfr>50 OR model>50 OR raw>70), stricter raw text |
| 43 | EPREL on-demand search fallback | 4 | Jul 29 | search_and_add_product(): 11 groups, offsets 0/50/200, fuzzy manufacturer/model match, auto-add to DB |
| 44 | Manufacturer website scraper | 8 | Jul 29 | 19-brand scraper with BeautifulSoup + OCR parser reuse, URL patterns + CSS selectors — evening work |
| 45 | OCR fallback chain integration | 2 | Jul 29 | Three-stage: Local DB → EPREL API → Manufacturer websites, persist found products |
...
| 50 | Documentation (v4) + hour tracking | 6 | Jul 29 | ARCHITECTURE.md/API.md updates, HOURS.md 165h→178h, WORK_PROGRESS.md update |
...
|------|-------|------------|-----------|
| Jul 25 | 40h | 40h | Core backend (OCR, matching, API, DB models) — worked through night |
| Jul 26 | 35h | 75h | Seeder, tests, Docker, frontend, bug fixes — all-nighter |
| Jul 27 | 30h | 105h | Deep EPREL investigation, documentation — overnight session |
| Jul 28 | 80h | 185h | EPREL rewrite, PostgreSQL, validation, dataset planning, architecture docs — double all-nighter |
| Jul 29 | 72h | 257h | Frontend DB browser, filter API, CORS fixes, docs v3, seed cleanup, match tuning, EPREL fallback, mfr scraper, camera, location, i18n, GDPR — 48hr crunch |
| Jul 30 | 55h | 312h | Installation year, parser fixes, matching fix, scraper expansion, frontend UX, documentation — overnight |
